...
const Users = () => {
  ...
  const edit = (staff) => {
    ...
    setForm({ name: staff.name || '', email: staff.email, password: '', role: staff.role, isActive: staff.isActive !== false });
  };
  ...
  return (
    <div className="min-h-screen bg-[#064e3b] text-white p-6 lg:p-8 -m-4 lg:-m-8">
      ...
      <form onSubmit={submit} className="bg-white/10 border border-white/15 rounded-2xl p-5 mb-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-3 items-end">
        ...
        <label className="text-sm text-gray-300">Role<input value={editingId ? form.role : 'Cashier'} readOnly className="mt-1 w-full rounded-lg bg-black/20 border border-white/20 px-3 py-2 text-white" /></label>
