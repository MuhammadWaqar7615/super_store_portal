...
const CategoryTable = ({ categories, onEdit, onDelete, readOnly = false }) => {
  return (
      ...
      <table className="min-w-full divide-y divide-white/10">
        <thead className="bg-white/5">
          <tr>
            ...
            {!readOnly && <th scope="col" className="px-6 py-4 text-right text-xs font-semibold text-gray-300 uppercase tracking-wider">Actions</th>}
          </tr>
        </thead>
        <tbody className="divide-y divide-white/10">
          ...
          {categories.length === 0 && (
            <tr>
              <td colSpan={readOnly ? 2 : 3} className="px-6 py-8 text-center text-sm text-gray-400">No categories found.</td>
            </tr>
          )}
        </tbody>
      </table>
